...
public class I_CoffeeManager {
	...
	public static void main(String[] args) {
		...
		// Coffee() : 생성자, Coffee타입의 인스턴스를 생성할 때 사용하는 특수한 메서드
		Coffee americano = new Coffee(); // 인스턴스화,(이결과로 만들어지는걸 인스턴스라고 함
		...
		System.out.println(americano.cost + "dddd");
		
		Coffee moca = new Coffee("모카", 4000, 3000, 10, 3, 0);
		System.out.println(moca.name);
		System.out.println(moca.stock);
			...
			if(inputMenu == 1) {
				...
				if(drinks[inputCode].stock < drinks[inputCode].safetyStock) {
					...
					if(balance > drinks[inputCode].safetyStock * 2 * drinks[inputCode].cost) {
						...
						//지출등록
						expences += drinks[inputCode].safetyStock * 2 * drinks[inputCode].stock;
