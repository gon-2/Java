...
public class _Map {
	...
	public void studyMap() {
		...
		// 3. Map에 저장된 데이터를 수정
		// put: 추가하는 경우 key가 map에 존재하지 않을경우 value를 추가
		// 존재하는 경우 key로 다른value에 매핑된 이전 값을 덮어쓴다.

		// replace : 특정된 key의 value값을 수정
		// 만약 key가 존재하지 않을 경우 null을 반환하고 종료한다.
		...
		List<Music> list = new ArrayList<>(map.values());
		System.out.println(list);

		Set<Music> set = new HashSet<>(map.values());
		System.out.println(set);
		// Set, List 두개다 같이 받아올 수 있다. 하지만 Set은 순서를 제대로 가져오진 않음
